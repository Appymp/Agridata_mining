...
##Collect multiple datasets saved in "datasets" folder and create 1 dataframe
...
for root, dirs, files in os.walk(working_dir):
    ...
    all_files+=file_list    
print(all_files) #list of file names
...
comb_df = comb_df.dropna(axis=0, subset=['description']) #Drop empty rows 
...
comb_df.rename(columns={"index": "ix"},inplace=True) #to ref after dropping rows
...
def desc_cleaning(org_str): #some hashtags are not separated by a space. 
...
def desc_splitting(df): 
  ...
  for ind,row in df_upd.iterrows():
      X=str(row['new_desc']).split()#ensure description column is cleaned prior
      X_cc=[]
      X_hstgs=[]
      X_cms=[]
      X_http=[]
...
def new_cols(df): ##wrapping above functions
...
# In[6]: Text preprocessing; fonts, lower, punct, char_encodings
...
# In[8]: "views" and "exemplars" - testing
##Define view frame to view previous processing steps with exemplars
view1=['description','new_desc','clean_captions' ]
view2=[]
view2=['clean_captions','caption_processed','caption_processed_2', 'caption_processed_3',
       'caption_processed_4']
...
ax= sns.countplot(x= 'det_lang', data=df_updated, order = df_updated['det_lang'].value_counts(
...
# In[10]: #Selective viewing of dataframe with "views" and "exemplars"
view_extracts=['new_desc','det_lang','clean_captions','caption_processed_4','hashtags',
...
# In[11]: ##Wordcloud with English and French stopwords

print("\nLoading test Word Cloud..")
start=datetime.now()

# df_updated['clean_captions'][10] #test individual rows
word_string = " ".join(df_updated['clean_captions'].dropna().astype(str)) #select column series
...
# In[12] Filter; read_csv("App_dataframe.csv") -> to_csv("App_dataframe_2.csv")
...
ad_4.rename(columns={"index": "ix"},inplace=True) #replace index to use as row ref
ad_4['ix']=ad_4['ix'].astype(str) #convert to string type

# ad_4.to_csv("App_dataframe_2.csv", index_label = False)


# In[13] More preprocessing; read_csv("App_dataframe_2.csv") --> to_pickle("App_dataframe_3.pkl")
...
# ad_4.to_pickle("App_dataframe_3.pkl", protocol=0) #preserves the list datatypes. prot=0 for colab
...
# In[14]# Class for cooccurence embeddings initial- from prof Redha Moulla notes
# Finally used different co-occ method with window selection.
...
# In[15]: #Test bag of words sizes and instantiate Coocc_Embedding class
...
# In[16]: #Lemmatization and Stemming ->create new pkl App_dataframe_4 
...
# In[17]: #More filtering of the dataframe App_dataframe_4->App_dataframe_5
...
ad_7.drop(['ix'], axis=1, inplace = True) #drop any older 'ix' columns
ad_7.reset_index(inplace= True,drop=True) #reset and drop old index
ad_7.reset_index(inplace= True) #create new ix column
ad_7.rename(columns={"index": "ix"},inplace=True) 

# ad_7.to_pickle("App_dataframe_5.pkl", protocol=0)

len(ad_7) #29681

# In[18]: #Window Cooccurence array; save('co_occ_arr.npy'); choose 'rm_sw_lemt'
...
# In[19]Perform Singular Value Decomposition on the array. SVD_matrix.npy
...
# co_occ_arr_sl=co_occ_arr[0:10000,0:10000] #sliced array
...
# In[20]: svd array->plotting dataframe
...
# vocab_to_plot = certs + fruits + rel_concs #specific plot list
vocab_to_plot = cust_vocab #full vocab plot
...
# In[21]: Visualise plotly scatter  - Coocc SVD vectors
...
# In[22]: ## Word2vec model and saving .txt files
import os
import re
import time
...
print("number of sentences is: ",len(words_rm_sw_lemt)) #41526
...
model.save("thesis_word2vec.model") #save a part of the training. avoid saving, is large
model = Word2Vec.load("thesis_word2vec.model") #load and continue training the sample
model.train([["hello", "world"]], total_examples=1, epochs=1) #train new samples if required
model.wv.most_similar('organic', topn = 20)
model.wv.save_word2vec_format('organic_glove_300d_w5.txt') #save all embeddings in this format

print("Time taken for creating model: ", str(datetime.now()-start)[:-5])#44 secs for 300d_w4

# In[23]: Create and Export Tsne dataframe by transforming w2c text files 
# Note how before the saving the model, called as model.wv 
# Takes 45-60 mins for full dataset.

w2v = KeyedVectors.load_word2vec_format('organic_glove_300d_w5.txt') # To load text file
# w2v = model.wv #use this if not loading from file and model is instantiated

def tsne_df(w2v): # dataframe conversion function
...
start=datetime.now()
tsne_df(w2v) #call the function

# In[24]: Addition and subtraction logic word2vec
w2v = KeyedVectors.load_word2vec_format('organic_glove_300d_w5.txt') # To load text file
w2v.most_similar(positive=['king','woman'], negative= ['man'])
print(w2v.most_similar(positive=['oil', 'hempseed'])) #addition
print(w2v.most_similar(['oil', 'hempseed','free'])) #same as above

# In[25]: App simulation - w2v Scatter plot logic including hue.
...
nearest_size = 5  #make input for nearest neighbor size
...
#index of 10 words closest to inputs
clos_ten_out = []
...
tsne_plot_df = tsne_df_full.loc[plot_index] #filter by index
tsne_plot_df
# tsne_plot_df = tsne_df_full[tsne_df_full['word'].isin(vocab_to_plot)] #filter by word
...
# In[26]: co-occurrence visualisations
#instantiate the sorted vocab in the co-occ code block
from numpy import load 
co_occ_arr = load('ad5_co_occ_arr_w5_bi.npy') #shape 41526 * 41526
...
import dash_table  #interactive datatable
import dash_core_components as dcc #display elements
import dash_html_components as html #display elements
from dash.dependencies import Input, Output, State #for app callbacks
import dash_bootstrap_components as dbc #display elements
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
from wordcloud import WordCloud, STOPWORDS
...
from gensim.models import KeyedVectors #For loading a saved model
from scipy import stats # for zscore
import numpy as np # for filtering zscore
from collections import Counter #co-occurence counts
import ast #literal evals for list types
...
try: #try and except for quick reloading of the w2v object.
    w2v
except:
    w2v = KeyedVectors.load_word2vec_format('organic_glove_300d_w5.txt') #initialise only once per app run

#Dashtables
df_updated = pd.read_pickle("df_8hrs.pkl") #filtered 8hr dataset
...
clf_table=pd.read_pickle("df_8hrs.pkl") #z-score table
...
t=datetime.now() - app_launch_start 
s=str(t) 
print("Dataframes loaded in: ", s[:-5], "\n\n")
...
print("Total time to load files and launch app", al[:-5])
...
col_sels_2 = ['description','hashtags','username','fullName']   #values for dropdown_2
        ...
        if len(chosen_rows)==0: #if no rows selected consider all rows of z score filtered table-defined again                   
            ...
            #-----                
                ...
                df_filtered['comb_cols'] = df_filtered[df_filtered.columns[0:]].apply( 
                    ...
                    axis=1)#Combine multiple columns into a single series 
                df_filtered['comb_cols'] = df_filtered['comb_cols'].apply(
                    lambda x: ' '.join(set(x.split())))#Use only unique words per post in wordcloud
                ...
                return (fig_br1)
        elif len(chosen_rows) > 0 : #filter to selected rows
            df_filtered = clf_dashtable_z[chosen_cols]
            df_filtered = df_filtered[df_filtered.index.isin(chosen_rows)]            
    elif len(chosen_cols) == 0:
        raise dash.exceptions.PreventUpdate #no update if no cols selected
    ...
    if len(chosen_cols) > 0: #atleast 1 col to be selected
        if len(chosen_rows)==0:       
            ...
            #-----            
                ...
                fig_wordcloud2 = px.imshow(wordcloud, template='ggplot2')#title="Post frequency"             
...
## load wordcloud only once
    df_filtered['comb_cols'] = df_filtered[df_filtered.columns[0:]].apply( #Combine multiple columns into a single series for wordcloud generation
        ...
        axis=1)
    df_filtered['comb_cols'] = df_filtered['comb_cols'].apply(
        lambda x: ' '.join(set(x.split()))) #because no order, bigrams become irrelevant
    ...
    fig_wordcloud_2 = px.imshow(wordcloud, template='ggplot2')#title="Post frequency"
    ...
    if len(chosen_cols) > 0: #atleast 1 col to be selected
        if len(chosen_rows)==0:                    
            ...
            if wc_init == 1: #load once.
                ...
                en_stopwords = stopwords.words('english')
                fr_stopwords = stopwords.words('french')
                web_links_sw = ['www','http','https','com','bit','co','ly']    
                combined_stopwords = web_links_sw + en_stopwords + fr_stopwords 
                print("\nNo of rows in WC are: ",len(df_filtered))
                wordcloud = WordCloud(max_words=100,    
                                       stopwords= combined_stopwords,
                                      colormap='tab20',
                ...
                fig_wordcloud1 = px.imshow(wordcloud, template='ggplot2')#title="Word frequency"            
...
## load wordcloud only once    
    ...
    wordcloud = WordCloud(max_words=100,    
                          ...
                          height=1000, #1000
                          random_state=1).generate(' '.join(df_filtered['comb_cols'])) #df_filtered has to be a series
    fig_wordcloud = px.imshow(wordcloud, template='ggplot2')#title="Word frequency"
